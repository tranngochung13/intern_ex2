			...
			foreach ( $items as $key => $val ) {
				...
				if ( isset( $val['from_timestamp'] ) && ( '' != $val['from_timestamp'] ) ) {
					...
					if ( isset( $finale_inventory_reduced_handle[ $pro_id ] ) && isset( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_out_key ] ) && ! empty( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_out_key ] ) ) {
						$temp_sold_out = (int) $val_data['quantity'];
					}
					if ( isset( $finale_inventory_reduced_handle[ $pro_id ] ) && isset( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_out ] ) && ! empty( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_out ] ) ) {
						$temp_sold_total_out = (int) $val_data['quantity'];
					}
					if ( isset( $finale_inventory_reduced_handle[ $pro_id ] ) && isset( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_unit_key ] ) && ! empty( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_unit_key ] ) ) {
						$temp_sold_unit = (int) $val_data['quantity'];
					}
					if ( isset( $finale_inventory_reduced_handle[ $pro_id ] ) && isset( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_unit ] ) && ! empty( $finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_unit ] ) ) {
						$temp_sold_total_unit = (int) $val_data['quantity'];
					}

					$finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_out_key ]    = ( $sold_unit_mod - $sold_unit ) + $temp_sold_out;
					$finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_out ]  = ( $total_sold_unit_mod - $total_sold_unit ) + $temp_sold_total_out;
					$finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_unit_key ]   = ( $sold_unit_mod ) + $temp_sold_unit;
					$finale_inventory_reduced_handle[ $pro_id ][ $wcct_sold_total_unit ] = ( $total_sold_unit_mod ) + $temp_sold_total_unit;
