...
class WCCT_Admin {
	...
	public function restrict_to_publish_when_campaign_is_disabled( $post_ID, $post_after, $post_before ) {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'wc-settings' && isset( $_GET['tab'] ) && $_GET['tab'] == WCCT_Common::get_wc_settings_tab_slug() ) {
		} else {
	...
	public function wcct_enqueue_admin_assets() {
		if ( true === WCCT_Common::wcct_valid_admin_pages() ) {
			wp_enqueue_style( 'xl-confirm-css', $this->get_admin_url() . 'assets/css/jquery-confirm.min.css', WCCT_VERSION );
			wp_enqueue_style( 'wcct_admin-css', $this->get_admin_url() . 'assets/css/wcct-admin-style.css', WCCT_VERSION );
			wp_enqueue_style( 'cmb2-styles' );
			wp_enqueue_script( 'xl-confirm-js', $this->get_admin_url() . 'assets/js/jquery-confirm.min.js', WCCT_VERSION );
			wp_enqueue_script( 'wcct_admin-js', $this->get_admin_url() . 'assets/js/wcct-admin.min.js', array( 'jquery', 'cmb2-scripts', 'wcct-cmb2-conditionals' ), WCCT_VERSION, true );
	...
	public function wcct_post_wcct_load_assets( $handle ) {
		...
		if ( ( $handle == 'post-new.php' || $handle == 'post.php' || $handle == 'edit.php' ) && ( $post_type == WCCT_Common::get_campaign_post_type_slug() || $post_type == 'countdown' ) ) {
			...
			wp_register_script( 'xl-ajax-chosen', $this->get_admin_url() . 'assets/js/chosen/ajax-chosen.jquery.min.js', array( 'jquery', 'xl-chosen' ), WCCT_VERSION );
	...
	public function wcct_wc_admin_menu() {
		...
		add_submenu_page( 'woocommerce', __( 'Campaigns', 'finale-woocommerce-sales-countdown-timer-discount' ), __( 'Campaigns', 'finale-woocommerce-sales-countdown-timer-discount' ), 'manage_woocommerce', 'admin.php?page=wc-settings&tab=' . WCCT_Common::get_wc_settings_tab_slug() . '', false, false );
	}
